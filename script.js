...
function generateChatMessage(userOverride, textOverride, colorOverride) {
   ...
   if (!user) {
       // Шанс на появление Модератора или VIP
       const roll = Math.random();
       if (roll < 0.05) {
           // 5% шанс на сообщение от модератора
           user = MOD_USERS[Math.floor(Math.random() * MOD_USERS.length)];
           text = MOD_MSGS[Math.floor(Math.random() * MOD_MSGS.length)];
           color = "#22c55e"; // Зеленый цвет ника
           isMod = true;
       } else if (roll < 0.08) {
           // 3% шанс на сообщение от VIP (Katherine)
           user = VIP_USER;
           text = VIP_MSGS[Math.floor(Math.random() * VIP_MSGS.length)];
           color = "#e83e8c"; // Розовый цвет ника
           isVip = true;
       } else {
   ...
   msgDiv.innerHTML = `${badgesHtml}<span class="chat-user" style="color: ${color};">${user}:</span><span class="chat-text">${text}</span>`;
